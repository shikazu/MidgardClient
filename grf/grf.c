...
GRFEXPORT bool grf_merge(grf_handle dest, grf_handle src, uint8_t repack_type)
{
    ...
    while(cur != NULL)
    {
        ...
        int i=0;
        for(i=0; *(rep->filename+i)!=0; i++) if (*(rep->filename+i)=='/') *(rep->filename+i)='\\';
        if (prev == NULL)
        {
            rep->pos = 0;
...
GRFEXPORT bool grf_repack(grf_handle handler, uint8_t repack_type)
{
    ...
    case GRF_REPACK_DECRYPT:
        break; // case GRF_REPACK_RECOMPRESS: break;
    default:
...
{
    size_t i;
    for(i=0; i<maxlen; i++) if (*(str+i)==0) return i;
    return maxlen;
}
...
static bool prv_grf_quicksort(struct grf_handler *handler, uint32_t elements)
{
    ...
    while (i>=0)
    {
        ...
        {
            piv=arr[L];
            if (i==PRV_GRF_QUICKSORT_MAX_LEVELS-1) return NULL;
            while (L<R)
            {
...
static bool prv_grf_load(struct grf_handler *handler)
{
    ...
    int i;
    for(i=1; i<=sizeof(head.header_key); i++) if ((head.header_key[i-1] != i) && (head.header_key[i-1] != 0)) return false;
    switch(head.version)
    {
    case 0x102:
    case 0x103:
...
//			if (handler->write_mode) return false; // disallow usage of "write mode" for those files. Repack is needed to write to those files.
        ...
        table = malloc(dlen);
        if (read(handler->fd, (void *)table, dlen) != dlen)
        {
            free(table);
            return false;
        }
        ...
        while(pos < pos_max)
        {
            ...
            if (*((entry->filename)+(fn_len-4)) == '.')
            {
                ...
                for(j=0; j<4; j++)
                {
                    ...
                    bool eq=true;
                    for(i=0; i<3; i++) if (ext[i]!=*(p+i)) eq=false;
                    if (eq)
                    {
                ...
                if (!nocrypt)
                {
                    ...
                    entry->cycle = 1;
                    for(i=10; (entry->len)>=i; i=i*10) entry->cycle++;
                }
                else
                {
    ...
    case 0xCACA: // broken-by-repack
        ...
        if (read(handler->fd, table_comp, posinfo[0]) != posinfo[0])
        {
            free(table);
            free(table_comp);
            return false;
        }
        if (read(handler->fd, (void *)&brokenpos, sizeof(uint32_t)) != sizeof(uint32_t))
        {
            free(table);
            free(table_comp);
            return false;
        }
        if (zlib_buffer_inflate(table, posinfo[1], table_comp, posinfo[0]) != posinfo[1])
        {
            free(table);
            free(table_comp);
            return false;
        ...
        while(pos < pos_max)
        {
            ...
            struct grf_table_entry_data tmpentry;
            result--;
            if (fn_len + sizeof(struct grf_table_entry_data) > av_len)
            {
                free(table);
                return false;
            }
...
GRFEXPORT grf_node grf_file_add(grf_handle handler, const char *filename, void *ptr, size_t size)
{
    ...
    int i;
    for(i=0; *(ptr_file->filename+i)!=0; i++) if (*(ptr_file->filename+i)=='/') *(ptr_file->filename+i)='\\';
    if (prev == NULL)
    {
        ptr_file->pos = 0;
...
static bool prv_grf_write_header(struct grf_handler *handler)
{
    ...
    uint8_t i;
    for(i=1; i<=sizeof(file_header.header_key); i++) file_header.header_key[i-1]=i;
...
static bool prv_grf_write_table(struct grf_handler *handler)
{
    ...
    *(uint32_t *)(pos) = table_size; /* compressed size */
    ...
    lseek(handler->fd, handler->table_offset + GRF_HEADER_SIZE, SEEK_SET);
    if (write(handler->fd, (char *)pos, table_size) != table_size)
    {
        free(pos);
        return false;
    }
...
GRFEXPORT bool grf_save(grf_handle handler)
{
    ...
    handler->filecount = handler->fast_table->count;
    if (prv_grf_write_table(handler) != true)
    {
        return false;
    }
    if (prv_grf_write_header(handler) != true)
    {
        return false;
    }
