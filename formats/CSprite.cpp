...
CSprite::CSprite(sf::String sFile, sf::Color *pPalette)
{
	FileStream flstream;
	GetPipe().getFileStream(sFile, flstream);
	construct(flstream, pPalette);
}
...
void CSprite::construct(FileStream &flstream, sf::Color *pPalette)
{
	uint16_t wSig;
...
const sf::Texture* CSprite::GetTexture(uint16_t index) const
{
	return vTextures.at(index);
}

const uint16_t CSprite::GetTextureCount() const
{
	return vTextures.size();
}

const bool CSprite::IsValid() const
{
	return bValid;
}
