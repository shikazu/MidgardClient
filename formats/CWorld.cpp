...
CWorld::CWorld(FileStream &flstream)
{
    ...
    if (lObjCount > 0)
    {
        ...
        {
            Obj* pObj = new Obj;
            flstream.read(&(pObj->lType), 2);
            switch(pObj->lType)
            {
                case OBJECT_MODEL:
                {
                    ObjModel* pModel = &(pObj->model);
                    if (wVersion >= 0x0103)
                    {
...
char* CWorld::GetIniName()
{
    return sIni;
}
char* CWorld::GetGndName()
{
    return sGnd;
}
char* CWorld::GetGatName()
{
    return sGat;
}
char* CWorld::GetScrName()
{
    return sScr;
}
uint32_t CWorld::GetObjCount()
{
    return vObjects.size();
}
CWorld::Obj* CWorld::GetObj(uint32_t dwIndex)
{
    return vObjects.at(dwIndex);
}
CWorld::QuadTreeNode* CWorld::GetNode(uint32_t dwIndex)
{
    if (dwIndex > 1364)
        dwIndex = 1364;
